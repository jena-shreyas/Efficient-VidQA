...
def main(args):
    ...
    if args.descriptive:
        ...
        with open(os.path.join(args.data_path, "descriptive_gt_wo_cxt.json"), "r") as h:
            gt_data = json.load(h)
        gt_qid2ans = {}
    ...
    if args.counterfactual:
        ...
        with open(os.path.join(args.data_path, "counterfactual_gt_wo_cxt.json"), "r") as h:
            gt_data = json.load(h)
        ...
        for k,v in pfms.items():
            ...
            if pfms[k]["po"]["total"]==0:
                print(f"We received zero total predictions for {k} action.")
                continue
